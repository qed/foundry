...
const EXAMPLE_PROMPTS = [
  'What work orders are blocked?',
  'Suggest a phase plan',
  'Summarize project progress',
  'Which items are high priority?',
]

export function FloorRightPanel({ open, projectId }: FloorRightPanelProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/agent/floor?projectId=${projectId}`)
        if (res.ok) {
  ...
  async function handleSend(content?: string) {
    ...
    try {
      const res = await fetch('/api/agent/floor/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId,
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && !isStreaming && (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              ...
              <p className="text-sm text-text-secondary mb-1">Assembly Floor Agent</p>
              <p className="text-xs text-text-tertiary mb-4">
                Ask me about work orders, phases, priorities, or project progress.
              </p>
