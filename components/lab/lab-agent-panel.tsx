...
import { AgentMessage } from '@/components/hall/agent-message'
import { AgentInput } from '@/components/hall/agent-input'
...
export function LabAgentPanel({ projectId, open, onClose }: LabAgentPanelProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/agent/lab?projectId=${projectId}`)
        if (res.ok) {
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border-default">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-text-primary">Insights Lab Agent</h2>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && !isStreaming && (
          <div className="flex flex-col items-center justify-center h-full text-center px-6">
            ...
            <p className="text-sm text-text-secondary mb-1">Insights Lab Agent</p>
            <p className="text-xs text-text-tertiary">
              Ask me to categorize feedback, find patterns, spot duplicates, or suggest which items to prioritize.
            </p>
          </div>
        )}
