...
interface TechnicalRequirementEditorProps {
  projectId: string
  docId: string
}

export function TechnicalRequirementEditor({ projectId, docId }: TechnicalRequirementEditorProps) {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          ...
          setTitle(data.title || 'Technical Requirement')
        }
      } catch (err) {
        console.error('Error loading technical requirement:', err)
